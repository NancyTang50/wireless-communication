...
async fn main() -> Result<()> {
    ...
    peripheral.start_advertising(ADVERTISE_NAME, &[service_uuid]).await.context("Failed to advertise").unwrap();
...
async fn make_peripheral() -> Result<(Uuid, Peripheral)> {
    ...
    peripheral.add_service(&service).context("Could not add service to peripheral")?;
