...
pub fn start_reading_sensor_data(mut temperature_changed_channel: Sender<f32>, mut humidity_changed_channel: Sender<f32>) 
{
    tokio::spawn(async move {
        ...
        loop {
            match dht22_pi::read(DHT_22_GPIO_PIN) {
                Ok(new_reading) => {
                    let (is_temperature_changed, is_humidity_changed) = is_reading_changed(new_reading, old_temperature, old_humidity);
                    
                    if is_temperature_changed {
                        match temperature_changed_channel.send(new_reading.temperature).await {
                            Ok(_) => {
...
fn is_reading_changed(new_reading: Reading, old_temperature: f32, old_humidity: f32) -> (bool, bool) {
    (
        is_not_within_threshold(new_reading.temperature, old_temperature, TEMPERATURE_THRESHOLD),
